...
def FBdatetime2timestamp(dtime):
    try:  # try to parse using standard US date format
        return str(int(time.mktime(time.strptime(
            dtime,
            ("%B %d, %Y at %I:%M %p" if
             dtime.endswith('am') or dtime.endswith('pm') else
             "%B %d, %Y at %H:%M")
        ))) * 1000)
...
def get_FB_albums(archive_reader):
    # Get the first entry in the file list ending with "photos.html"
    photos_page_name = next(name for name in archive_reader.namelist()
                            if name.endswith('photos.html'))

    album_root_path = posixpath.dirname(photos_page_name) + '/'
    photos_page = dom.parseString(
        archive_reader.read(photos_page_name).replace('<BR>', '<br/>')
    )
    albums = {}
    for container in photos_page.getElementsByTagName('div'):
        if container.getAttribute('class') == 'album':
            parsed_album = FBAlbum(container, album_root_path)
            albums[parsed_album.title] = parsed_album
    return albums


def get_FB_album_photos(archive_reader, album):
    photo_root_path = posixpath.dirname(album.path) + '/'
    album_page = dom.parseString(
        archive_reader.read(album.path).replace('<BR>', '<br/>')
    )
    photos = []
...
class FBAlbum(object):
    def __init__(self, container, root_path=""):
        ...
        self.path = posixpath.normpath(
            root_path + urllib.unquote(extract_path(links[0]))
        )
        self.title = unicode(links[1].firstChild.nodeValue)
        self.cover_photo_path = extract_path(
            container.getElementsByTagName('img')[0], 'src'
        )

        for elem in container.childNodes:
            if (
                elem.nodeType != dom.Node.TEXT_NODE and
                elem.tagName == 'span' and
                elem.getAttribute('class') == 'time'
            ):
...
class FBPhoto(object):
    def __init__(self, container, root_path=""):
        ...
        self.datetime = links[2].getElementsByTagName('span')[0]\
        ...
        for br in line_breaks:
            if (
                br.nextSibling.nodeValue and
                br.nextSibling.nodeValue.strip() == 'In this video:'
            ):
                ...
                while span_element != next_br:
                    if (
                        span_element.nodeType != dom.Node.TEXT_NODE and
                        span_element.tagName == 'span'
                    ):
        ...
        containers = (comment_container for comment_container in
                      container.getElementsByTagName('div')
                      if comment_container.getAttribute('class') == 'comment')
        self.comments = map(FBComment, containers)
...
class FBComment(object):
    def __init__(self, container):
        ...
        while node != container.lastChild:
            ...
            elif (node.nodeType == dom.Node.ELEMENT_NODE and
                  node.tagName.lower() == 'br'):
                self.message += "\n"
